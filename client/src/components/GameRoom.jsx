...
export default function GameRoom() {
  ...
  const bonusButtonRef = useRef(null);
  ...
  // Flying coin animation when >= 3 dots captured
  useEffect(() => {
    if (!lastCaptureCoinInfo || lastCaptureCoinInfo.playerIndex !== myPlayerIndex) return;

    const boardEl = boardWrapperRef.current;
    const bonusEl = bonusButtonRef.current;
    if (!boardEl || !bonusEl) return;
    ...
    const toX = bonusRect.left + bonusRect.width / 2;
    const toY = bonusRect.top + bonusRect.height / 2;

    // Arc midpoint: go up first
    const midX = (fromX + toX) / 2;
    const midY = Math.min(fromY, toY) - 120;

    setFlyingCoin({
      id: lastCaptureCoinInfo.time,
    ...
    const timer = setTimeout(() => setFlyingCoin(null), 1400);
    return () => clearTimeout(timer);
  }, [lastCaptureCoinInfo, myPlayerIndex, playCoinSound]);
  ...
  return (
    <>
      {/* Server reconnecting overlay */}
      {(isReconnecting || !isConnected) && !!gameState && !gameOverResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-4 bg-slate-800 rounded-2xl p-8 shadow-2xl border border-slate-600">
            <div className="w-12 h-12 border-4 border-blue-400 border-t-transparent rounded-full animate-spin" />
            <p className="text-white text-lg font-semibold">{t('game.server_reconnecting')}</p>
            <p className="text-slate-400 text-sm">{t('game.server_reconnecting_sub')}</p>
          </div>
        </div>
      )}
      ...
      >
        <div className={`absolute inset-0 transition-all duration-300 mix-blend-screen ${flashCapture === 'green'
      ...
      <AnimatePresence>
        {flyingCoin && (
          <motion.div
            key={flyingCoin.id}
            initial={{
              left: flyingCoin.from.x - 24,
              top: flyingCoin.from.y - 24,
              scale: 2,
              opacity: 1,
            }}
            animate={{
              left: [flyingCoin.from.x - 24, flyingCoin.mid.x - 24, flyingCoin.to.x - 24],
              top: [flyingCoin.from.y - 24, flyingCoin.mid.y - 24, flyingCoin.to.y - 24],
              scale: [2, 1.8, 0.6],
              opacity: [1, 1, 0.4],
            }}
          ...
          >
            <div className="text-5xl drop-shadow-[0_0_20px_rgba(234,179,8,0.9)] filter brightness-125">🪙</div>
          </motion.div>
        )}
      </AnimatePresence>
                ...
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`flex items-center justify-between px-4 py-3 rounded-xl font-bold transition-all active:scale-95 disabled:opacity-60 ${color === 'purple' ? 'bg-purple-600 hover:bg-purple-500 text-white' :
      ...
      <motion.div
        ...
        className={`w-full flex flex-row gap-2 md:gap-6 h-[calc(100vh-8rem)] max-md:h-[calc(100dvh-1rem)] relative z-10 transition-transform duration-100 ${flashCapture === 'red' ? 'scale-[0.98] translate-y-1 rotate-[0.5deg]' :
      ...
      >
        ...
        <div className="flex-1 flex flex-col min-w-[60%] max-md:min-w-0 bg-slate-800/80 rounded-3xl border border-slate-700 p-3 md:p-6 shadow-[0_0_40px_-15px_rgba(0,0,0,0.5)] relative backdrop-blur-xl overflow-hidden min-h-0">
          ...
          {/* Game Board Area */}
          <div ref={boardWrapperRef} className={`flex-1 min-h-0 flex items-center justify-center relative rounded-2xl border overflow-hidden transition-all duration-500 delay-75 ${flashCapture === 'green' ? 'bg-green-900/40 border-green-500 shadow-[inset_0_0_80px_rgba(74,222,128,0.6)]' :
            ...
            <GameBoard
              ...
              persistedContours={persistedContours}
          ...
          <div className="shrink-0 mt-4 flex flex-col gap-2">
            <div className="flex justify-center gap-2 flex-wrap max-md:grid max-md:grid-cols-4 max-md:gap-1.5">
              ...
              <button
                ...
                className={`flex-1 max-w-[110px] max-md:max-w-none flex justify-center items-center gap-1.5 max-md:flex-col max-md:gap-1 px-3 max-md:px-1 py-2.5 max-md:py-3 rounded-xl font-bold transition-all shadow-lg active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed text-sm max-md:text-xs ${drawOfferSent
